...
export class EquipmentsUseCases {
  ...
  async getEquipmentsByFilter(query: any): Promise<Equipments[]> {
    ...
    const isInvalidParam = keys.find(
      (key) => !EquipmentParamsConstants.includes(key),
    );
    const isMissingValues = values.find((value) => !value);

    if (isInvalidParam || isMissingValues === '')
      throw new Error('Invalid Query of Equipments');

    const buildFilterDataAndValidate = this.filterService.createfilter(query);

    return await this.filterRepository.getAllByFilter(
      'equipments',
      buildFilterDataAndValidate,
    );
  }
  ...
  async updateEquipment(
    id: string,
    equipment: Partial<Equipments>,
  ): Promise<Equipments | false> {
